...
import (
	...
	rpc "github.com/worldopennetwork/go-won/rpc"
...
func New(ctx *node.ServiceContext, config *won.Config) (*LightEthereum, error) {
	chainDb, err := won.CreateDB(ctx, config, "lightchaindata")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEthereum{
		...
		engine:           won.CreateConsensusEngine(ctx, &config.Ethash, chainConfig, chainDb),
		shutdownChan:     make(chan bool),
		networkId:        config.NetworkId,
		bloomRequests:    make(chan chan *bloombits.Retrieval),
		bloomIndexer:     won.NewBloomIndexer(chainDb, light.BloomTrieFrequency),
...
func (s *LightDummyAPI) Wonbase() (common.Address, error) {
...
func (s *LightEthereum) APIs() []rpc.API {
	return append(wonapi.GetAPIs(s.ApiBackend), []rpc.API{
		{
			Namespace: "won",
			Version:   "1.0",
...
func (s *LightEthereum) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = wonapi.NewPublicNetAPI(srvr, s.networkId)
