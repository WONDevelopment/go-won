...
func NewLesServer(won *won.WorldOpenNetwork, config *won.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(won.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, won.EventMux(), won.Engine(), newPeerSet(), won.BlockChain(), won.TxPool(), won.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(won.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(won.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(won.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(won.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(won.ChainDb())
	return srv, nil
}
...
func newCostStats(db wondb.Database) *requestCostStats {
