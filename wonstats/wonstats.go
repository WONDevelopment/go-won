...
func New(url string, ethServ *won.WorldOpenNetwork, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["won"]; info != nil {
		network = fmt.Sprintf("%d", info.(*won.NodeInfo).Network)
		protocol = fmt.Sprintf("won/%d", won.ProtocolVersions[0])
	} else {
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.won != nil {
		...
		if block == nil {
			block = s.won.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.won.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.won != nil {
			head = s.won.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.won != nil {
			block = s.won.BlockChain().GetBlockByNumber(number)
		} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.won != nil {
		mining = s.won.Miner().Mining()
		hashrate = int(s.won.Miner().HashRate())

		sync := s.won.Downloader().Progress()
		syncing = s.won.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price := s.won.ApiBackend.FixedPrice()
		if price.Int64() == 0 {
			price, _ = s.won.ApiBackend.SuggestPrice(context.Background())
		}
