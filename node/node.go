...
func (n *Node) OpenDatabase(name string, cache, handles int) (wondb.Database, error) {
	if n.config.DataDir == "" {
		return wondb.NewMemDatabase()
	}
	return wondb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
