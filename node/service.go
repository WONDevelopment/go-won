...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (wondb.Database, error) {
	if ctx.config.DataDir == "" {
		return wondb.NewMemDatabase()
	}
	db, err := wondb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
