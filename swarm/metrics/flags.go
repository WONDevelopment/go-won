...
func Setup(ctx *cli.Context) {
	if gwonmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gwonmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
