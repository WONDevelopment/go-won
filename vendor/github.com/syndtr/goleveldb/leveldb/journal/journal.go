...
//
// There are four chunk types: whwon the chunk is the full journal, or the
...
type Reader struct {
	...
	n int
	// last is whwon the current chunk is the last chunk of the journal.
	last bool
...
type Writer struct {
	...
	written int
	// first is whwon the current chunk is the first chunk of the journal.
	first bool
	// pending is whwon a chunk is buffered but not yet written.
	pending bool
	// err is any accumulated error.
	err error
	// buf is the buffer.
