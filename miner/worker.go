...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, won Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		won:            won,
		...
		chain:          won.BlockChain(),
		proc:           won.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(won.BlockChain(), miningLogAtDepth),
	}
	// Subscribe TxPreEvent for tx pool
	worker.txSub = won.TxPool().SubscribeTxPreEvent(worker.txCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = won.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = won.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) setWonbase(addr common.Address) {
	self.mu.Lock()
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				if self.config.Dpos != nil {
					if self.current == nil {
						log.Error("self.current == nil", "self", self)
						continue
					} else if self.current.signer == nil {
						log.Error("self.current.signer == nil", "current", self.current)
						continue
					}
				}
				self.currentMu.Lock()
