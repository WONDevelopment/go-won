...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gwon.LightScryptN, Gwon.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			WorldOpenNetworkClient ec = node.getEthereumClient();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gwon-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/wontest", "libs"} {
	...
	build.CopyFile(filepath.Join("libs", "gwon.aar"), "gwon.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "wontest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
