...
type Database struct {
	diskdb wondb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb wondb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) Node(hash common.Hash) ([]byte, error) {
	...
	//log.Debug("Database diskdb.Get Node  for " , "hash", hash)
...
func (db *Database) commit(hash common.Hash, batch wondb.Batch) error {
	// If the node does not exist, it's a previously committed node
