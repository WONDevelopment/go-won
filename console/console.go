...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute WorldOpenNetwork requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var won = web3.won; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = won.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines won.getBalance(won.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gwon instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gwon metadata
	fmt.Fprintf(c.printer, "Welcome to the Gwon JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + won.coinbase);
		console.log("at block: " + won.blockNumber + " (" + new Date(1000 * won.getBlock(won.blockNumber).timestamp) + ")");
