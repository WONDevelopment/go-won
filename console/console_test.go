...
func newTester(t *testing.T, confOverride func(*won.Config)) *tester {
	...
	// Create a networkless protocol stack and start an WorldOpenNetwork service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return won.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register WorldOpenNetwork protocol: %v", err)
	}
