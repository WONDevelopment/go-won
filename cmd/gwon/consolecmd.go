...
// localConsole starts a new gwon node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc gwon: %v", err)
	}
...
// remoteConsole will connect to a remote gwon instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running gwon instance and start the JavaScript console
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gwon: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "gwon attach" and "gwon monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gwon < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new gwon node, attaches an ephemeral JavaScript
