...
import (
	...
	swarmmetrics "github.com/worldopennetwork/go-won/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the WorldOpenNetwork API provider to use to settle SWAP payments",
...
// This init function sets defaults so cmd/swarm can run alongside gwon.
func init() {
...
var app = utils.NewApp(gitCommit, "WorldOpenNetwork Swarm")
...
func bzzd(ctx *cli.Context) error {
	...
	//gwon only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gwon and swarm
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *wonclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = wonclient.Dial(bzzconfig.SwapApi)
			if err != nil {
