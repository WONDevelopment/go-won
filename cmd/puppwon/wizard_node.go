...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an wonbase to mine against
			fmt.Println()
			if infos.wonbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.wonbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.wonbase)
				infos.wonbase = w.readDefaultAddress(common.HexToAddress(infos.wonbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy WorldOpenNetwork node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
