...
// setting up a new or managing an existing WorldOpenNetwork private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppwon, your WorldOpenNetwork private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new WorldOpenNetwork network down to  |")
	...
	log.Info("Administering WorldOpenNetwork network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppwon", w.network)
