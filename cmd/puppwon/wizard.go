...
// config contains all the configurations needed by puppwon that should be saved
// between sessions.
type config struct {
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppwon configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // WorldOpenNetwork services known to be running on servers
...
// it to an WorldOpenNetwork address.
...
// converts it to an WorldOpenNetwork address. If an empty line is entered, the default
// value is returned.
