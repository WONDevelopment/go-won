...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the WorldOpenNetwork protocol")
	statsFlag   = flag.String("wonstats", "", "Wonstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for WorldOpenNetwork and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // WorldOpenNetwork address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an WorldOpenNetwork light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // WorldOpenNetwork protocol stack
	client *wonclient.Client   // Client connection to the WorldOpenNetwork chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the WorldOpenNetwork light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := won.DefaultConfig
	...
	// Assemble the wonstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEthereum
			ctx.Service(&serv)
			return wonstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := wonclient.NewClient(api)
...
// close terminates the WorldOpenNetwork connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the WorldOpenNetwork address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and WorldOpenNetwork address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for WorldOpenNetwork addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No WorldOpenNetwork address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the WorldOpenNetwork address and profile URL.
...
// returning the username, avatar URL and WorldOpenNetwork address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain WorldOpenNetwork address,
