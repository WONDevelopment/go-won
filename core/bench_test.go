...
// genTxRing returns a block generator that sends won in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db wondb.Database
	if !disk {
		db, _ = wondb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "won-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = wondb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db wondb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "won-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := wondb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
