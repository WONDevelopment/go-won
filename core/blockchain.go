...
// Package core implements the WorldOpenNetwork consensus protocol.
package core
...
type BlockChain struct {
	...
	db     wondb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default WorldOpenNetwork Validator and
// Processor.
func NewBlockChain(db wondb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= wondb.IdealBatchSize {
	...
	for i, block := range chain {
		...
		case CanonStatTy:
			log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()),
				"txs", len(block.Transactions()), "gas", block.GasUsed(), "elapsed", common.PrettyDuration(time.Since(bstart)), "diff", block.Difficulty(), "nonce", block.Nonce())
